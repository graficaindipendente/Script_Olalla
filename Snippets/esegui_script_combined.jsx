...
// Funzione combinata che richiama più funzioni
function combinedFunction() {
    unlockAllLayers();
    deleteEmptyLayers();
    deleteElementsOutsideArtboard();
    saveAsPDFWithHighQuality();
}
...
// Aggiunge caselle di controllo con descrizioni a caporiga e righe vuote
var checkboxes = [];
for (var i = 0; i < scripts.length; i++) {
    var group = scriptPanel.add("group");
    group.orientation = "column";
    group.alignChildren = "left"; // Allinea il gruppo a sinistra
    var checkbox = group.add("checkbox", undefined, scripts[i].name);
    checkbox.value = true; // Pre-seleziona tutte le caselle di controllo
    checkboxes.push(checkbox);
    var description = group.add("statictext", undefined, scripts[i].description);
    description.graphics.font = "dialog:10";
    var emptyLine = group.add("statictext", undefined, ""); // Riga vuota
}
